...
namespace VUA_App.ViewModels
{
    class SubjectViewModel : BaseViewModel
    {
        ...
        async void ExecuteLoadSubjectsCommand(dynamic fetchParams)
        {
            ...
            if (searchTerm != null) subjects = await DataFetcher.GetInstance().GetSubjectSearchResultsByTypeAsync(faculty, searchTerm, isOptional, isBUS); //GetSubjectSearchResultsByType
            else if (faculty != Faculty.None)
            {
                if (isBUS) subjects = await DataFetcher.GetInstance().GetBUSSubjectsAsync(faculty);
                else subjects = await DataFetcher.GetInstance().GetSubjectsByTypeAndFacultyAsync(isOptional, faculty);
            }
            else subjects = await DataFetcher.GetInstance().GetSubjectsByTypeAsync(isOptional, isBUS); //GetSubjectsByType
            Subjects.Clear();
