...
namespace VUA_App.ViewModels
{
    class LecturerViewModel : BaseViewModel
    {
        ...
        async Task ExecuteLoadLecturersCommand(dynamic fetchParams) {
            ...
            if (searchTerm != null) lecturers = await DataFetcher.GetInstance().GetLecturerSearchResults(searchTerm, faculty);
            else if (faculty != Faculty.None) lecturers = await DataFetcher.GetInstance().GetLecturersByFaculty(faculty);
            else lecturers = await DataFetcher.GetInstance().GetLecturers();
            Lecturers.Clear();
